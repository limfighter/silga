...
```
리포 내 유일한 테스트 — Playwright로 검색→스펙필터→빌드생성→상세→목록→홈
→통계→최근기록→즐겨찾기 흐름을 실브라우저로 검증하는 단일 선형 스크립트.
개별 테스트 필터링은 없음. 실제 스크래핑을 그대로 태우므로 전체 2~4분 걸림.
검증 28건 전부 통과하면 exit 0, 하나라도 실패하면 실패 지점 스크린샷을
남기고 exit 1. 만든 빌드는 끝에서 지우므로(실패해도 지움) 반복 실행 가능.

환경변수는 전부 선택: `E2E_BASE`/`E2E_API`(주소), `E2E_SHOT_DIR`(스크린샷
폴더, 기본 `./e2e-shots`), `E2E_CHROMIUM`(playwright가 받아둔 브라우저와
버전이 안 맞는 환경에서만 실행 파일 경로 지정), `E2E_KEEP_BUILD=1`(뒷정리
