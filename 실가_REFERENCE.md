...
verdict 판정 기준가(basis_price) — 이동평균 도입 (2026-08-04, v0.4 예정)
  ...
  이동평균 계산 (services/verdict.py::compute_ma_price):
    - danawa.get_price_variance(code, 1)(1개월치 daily 시계열, 7/14/30일
      전부 커버)을 재사용, 신규 스크래핑 함수 없음
    - N(기간)은 쿼리파라미터/요청필드 ma_window로 선택: 7 | 14 | 30, 기본 14
    - "엄격" 원칙: 선택한 N일치 daily 데이터가 정확히 다 있어야 유효(모자라면
      그 부품의 이동평균은 무효=None)
    - 정렬은 리스트 원본 순서를 신뢰하지 않고 매번 full_date로 명시 정렬 후
      마지막 N개 사용 — 다나와 API의 실제 정렬 순서(오름차순/내림차순)를
      개발 환경에서 라이브 검증하지 못했음(네트워크 정책상 danawa.com 접근
      차단), full_date 없는 항목이 섞이면 정렬 신뢰 불가로 보고 무효 처리.
      **로컬 환경에서 실제 응답 포맷을 반드시 한 번 확인할 것**
