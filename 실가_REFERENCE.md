...
코드 안 버전 문자열도 같이 올린다 (2026-08-23 명문화):
  - backend/app/main.py 의 FastAPI(version=...)  ← /openapi.json에 노출됨
  - frontend/package.json 의 "version"
  둘 다 인수인계 버전 표와 같은 값으로 맞출 것. 초기값 "0.1.0"이 backend
  v0.16까지 방치돼 있어서 AI 라우터가 보는 API 스펙에 0.1.0이 나가고
  있었음 — 같은 일이 반복되지 않게 규칙에 못박음

Git 커밋 기준:
  - 두 번째 자리 이상 변경 시 커밋 필수
  - 세 번째 자리 변경은 커밋 선택
  - (삭제된 규칙) "bak 파일은 두 번째 자리 변경 시만 생성"은 2026-08-23에
    없앰 — 규칙이 있는 동안 실제로 만들어진 .bak 파일이 0개였고, git이
    있는 이상 필요도 없음. 지켜지지 않는 규칙을 문서에 남겨두면 다음
    세션이 "왜 안 지켰지"로 시간을 쓰게 됨
...
**"축적 금지"의 경계 — 아래 둘은 이 원칙 위반이 아님:**
- `_verdict_basis_cache` — (build_id, ma_window) 5분 프로세스 메모리 캐시
  (2026-08-04). DB 저장이 아니고 프로세스가 죽으면 사라짐
- `list_builds`의 요청 단위 중복 제거 dict(`price_memo`/`ma_memo`, 2026-08-22)
  — TTL도 프로세스 간 공유도 없고, **응답 하나를 만드는 동안** 같은 상품을
  두 번 긁지 않기 위한 것. 저장된 빌드를 전부 순회하는 구조라 같은 부품이
  여러 빌드에 들어 있으면 그 수만큼 중복 조회가 나던 것을 막음
  (실측: 빌드 5개 × 부품 8개에서 다나와 호출 80회 → 48회, 응답 동일)
- 사용자 결정(2026-08-22): 가격 변동이 잦아서 하루에 여러 번 켜는 게 아니면
  캐시 체감이 크지 않다고 보고, **가격 캐시/저장은 계속 도입하지 않음**
...
배포       확정(2026-08-07) — GCP Compute Engine e2-micro VM 단일 인스턴스.
           사용자가 GCP를 자주 다뤄봐서 익숙하다는 이유로 선택 — Fly.io/
           Railway(마찰은 더 적었을 후보)보다 우선. 프론트(정적 빌드)+
           백엔드(FastAPI) 같은 VM 하나에서 서빙(nginx가 정적파일 서빙 +
           `/api/`만 uvicorn으로 리버스 프록시) — 이 구성이면 프론트가
           같은 오리진으로 호출하게 되어 CORS 자체가 불필요해짐(프로덕션
           한정, 로컬 dev는 여전히 크로스 오리진이라 CORS 필요). 도메인/
           HTTPS는 아직 없음 — VM 외부 IP로 HTTP 직접 접속, 도메인 생기면
           그때 certbot 추가 예정. 셋업 절차/systemd·nginx 설정은
           `deploy/` 참조
           **리전 2026-08-15 변경: us-central1(무료 티어) → asia-northeast3
           서울(유료, GCP 90일 $300 체험 크레딧으로 운용)** — 사용자·
           스크래핑 왕복 지연 개선이 목적. 크레딧은 같은 결제 계정의 다른
           워크로드와 공유돼 1.5~2개월 내 소진 전망이라 임시 배치로 취급 —
           소진 임박 시 us-central1 무료 티어로 되돌릴 것(`deploy/README.md`
           "9. 비용 확인 + 크레딧 소진 전 되돌리기" 참조). 비용을 예산
           (하루 300~500원)에 맞추려고 VM을 24시간이 아니라 08:00~20:00
           (KST) 인스턴스 스케줄로만 자동 가동 + 필요시 수동 on/off. RAM
           2GB(e2-small) 업그레이드는 이 예산으로는 24시간 상시 기준
           안 되고, e2-micro(RAM 1GB) 유지 + 스왑 파일 2GB로 OOM 완화.
           외부 IP는 고정(static) 승격 — ephemeral IP는 스케줄로 매일
           껐다 켤 때마다 바뀔 수 있어서. 단 고정 IP는 인스턴스에 붙어
           있으면 VM이 꺼져있어도 24시간 과금이라, 스케줄 12시간분
           컴퓨팅·디스크 + IP 24시간분 합산 시 약 395원/일(예산 안)
           (기각된 후보, 참고용)
...
GET  /builds?ma_window={7|14|30}  (신규, v0.3)
  ...
  ※ with_prices=false(v0.18) — 스크래핑을 아예 건너뛰고 DB에 있는 구조
    (이름/부품수/저장일)만 즉시 반환. 가격·판정 필드는 전부 null이 된다.
    기본값 true라 기존 호출부(AI 라우터 등)는 영향 없음

GET  /builds/prices?ma_window={7|14|30}  (v0.18 신설)
  → [{id, total_price, total_price_formatted, verdict, verdict_confidence,
      ma_window, diff_percent}, ...]
  ※ 위 with_prices=false와 짝. 구조를 먼저 그린 뒤 가격만 따로 받아 id로
    채워 넣는 2단계 로딩용. 필드 의미는 BuildSummary의 동명 필드와 동일하고
    계산 로직도 _build_price_summary()로 공유하므로 두 경로의 값이 항상 같다
  ※ **요청을 빌드마다 쪼개지 않은 것은 의도적** — 프론트가 카드별로 호출하면
    브라우저가 동시에 날려서 매너 크롤링 원칙(동시 병렬 지양)을 깬다. 그래서
    스크래핑은 서버가 한 요청 안에서 순차로 돌리고, 나눈 것은 "구조 / 가격"
    두 단계뿐이다. 총 요청 수와 소요 시간은 그대로이고 첫 화면이 뜨는 시점만
    당겨진다(실측: 앱 부팅 후 카드 0.06초, 가격 5.9초 뒤 도착)
  ※ 라우트는 반드시 /builds/{build_id}보다 먼저 정의할 것 — 뒤에 두면
    "prices"가 build_id로 잡혀 422가 난다
...
├─ 통계      — ✅ 완료, SVG 라인차트(GET /product/{code}/history)
│              + PartRow로 부품 검색 + 1/3/6/12개월 탭
│              추세 요약 한 줄 + y축 금액 라벨 + 접히는 데이터 표(v0.19)
...
- **SVG 차트는 같은 데이터를 읽을 수 있는 경로를 반드시 함께 둔다**
  (2026-08-23 명문화). 홈 판정 축은 .axis-list, 통계 차트는 .ch-table-wrap
  (<details> 기본 접힘) — 그림만 있으면 스크린리더에는 빈 요소 하나다.
    통계 차트 y축 금액 라벨은 SVG가 아니라 HTML로 얹는다. 이 SVG는
    preserveAspectRatio="none"으로 가로를 늘려 그리므로 <text>를 안에 넣으면
    글자까지 늘어난다 — 안에 넣으려면 stretch를 포기하고 buildPoints()를
    실제 렌더 폭 기준으로 다시 짜야 한다
    관측이 MIN_POINTS_FOR_LINE(=4) 미만이면 꺾은선 대신 수치를 나열한다.
    점 2개는 무슨 값이든 완벽한 직선이라 추세처럼 보이지만 추세가 아님
    한 카드 안에서 같은 이름표로 다른 값을 보여주지 말 것 — legend의 "현재"는
    상품 실시간 최저가이고 요약의 값은 마지막 주 단위 관측치라 서로 다르다.
    후자는 "마지막 관측"으로 부른다
...
인쇄
  빌드 상세는 견적서로 출력 가능해야 함 — @page margin 상하10mm·좌우8mm
  (2026-08-13 조정, 이전 12mm 균등), 사이드바/탑바/버튼/sticky 요약/카트는
  @media print에서 숨김, .spec-row/.strip/.sum/.stack-bar/.answer/.axis는
  break-inside:avoid
  **여백값과 "종이가 많이 남아 보임"은 서로 다른 축이다 (2026-08-13)** —
  Chromium(page.pdf)으로 12/10/10+8/8/6mm 5개 안을 실측 비교한 결과, 부품
  8종 견적서는 12mm 기준으로도 페이지 하단이 40mm 이상 빈다. 이건 내용
  높이가 A4 한 장을 못 채워서 생기는 죽은 공간이라 margin 문제가 아니고,
  오히려 margin을 좁힐수록 인쇄 가능 영역만 넓어져 죽은 공간이 더 커진다
  (12mm일 때 40.6mm 빈 공간 → 6mm일 때 52.6mm). margin 조정은 "테두리가
  좁아 보이는" 느낌만 개선함 — 하단 여백 자체를 줄이려면 부품 수가 적은
  견적서에서의 체감을 별도로 확인하고 다른 방향(내용 밀도/레이아웃)으로
  접근해야 함. 6mm 균등안은 가정용 프린터 비인쇄 하한선(보통 4~5mm)에
  가까워 테두리 잘림 위험으로 보류, 10mm/8mm(상하/좌우)로 채택.
