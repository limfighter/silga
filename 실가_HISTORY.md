...
## 2026-08-23 (이어서) 썩은 목업 2개 삭제 + 죽은 코드 정리 + ±5% 확정

[✓] app-shell-mockup.html / silga-mockup.html 삭제 (git show 77ca8ce:<파일명>)
    → 2026-08-01 작성분. 2026-08-05 개편으로 톤이 통째로 바뀌었는데도
      CLAUDE.md·REFERENCE.md가 "API 계약의 근거"로 계속 지목하고 있었음
    → 계약 정본을 REFERENCE #엔드포인트-설계로 명시하고,
      api.ts ↔ schemas/*.py ↔ 그 문서의 삼각 대응으로 문구 교체
    → 과거 기록(HISTORY, 인수인계 이전 섹션)의 언급은 손대지 않음 —
      그 시점엔 사실이었던 서술이라 append-only 원칙에 어긋난다

[✓] api.ts listBuilds 제거 — 호출처 0곳
    → 백엔드 GET /builds?with_prices=true는 유지(AI 라우터 소비자가 있음)

[✓] VERDICT_THRESHOLD_PERCENT ±5% — 미결 → 확정(변경 없음)
    → 판정 라벨은 요약이고 실제 판단은 사용자가 diff_percent를 보고 한다.
      기준가 흔들림은 이동평균 기간(7/14/30일)이 이미 보완하고 있음

[교훈] "나중에 쓸지도 모르니 남겨둠"이 문서 거짓말로 굳는다
    목업 2개도, listBuilds도 각각 "유지", "기존 방식으로 남겨둠"이라고
    적어두고 방치한 것이었다. 코드는 죽은 채로 남으면 그냥 노이즈지만,
    문서가 그걸 "현재 유효한 근거"로 가리키고 있으면 다음 세션을 적극적으로
    잘못된 방향으로 이끈다. 남길 거면 왜 남기는지와 언제 지울지를 같이
    적고, 그게 안 써지면 그냥 지우는 게 맞다.

[교훈] 미결 항목에도 "닫는 조건"이 필요하다
    ±5%는 세션마다 "검증되지 않은 가정값"으로 되살아나고 있었다. 애초에
    실측으로 검증할 수 있는 종류의 값이 아니었는데(사용자 취향 영역),
    "검증 필요"로 적어둔 탓에 영원히 안 닫히는 항목이 됐다. 이번에
    "판단값으로 확정, 다시 미결로 올리지 말 것"까지 문서에 박았다.

---

## 2026-08-23 (이어서) 케이스·파워·쿨러 필터 확장 — backend v0.20 / frontend v0.20

세 카테고리만 필터가 2개씩이라 다른 카테고리(3~4개)보다 빈약했던 것을 메움.
전부 4개로 맞췄다(GPU와 같은 상한).

| 카테고리 | 추가 | 속성코드 | 실측/채택 |
|---|---|---|---|
| 케이스 | max_vga_length | 1009 | 10 → 7 |
| 케이스 | max_cooler_height | 21573 | 9 → 9 |
| 파워 | cable_type | 40062 | 3 → 3 |
| 파워 | formfactor | 1086 | 9 → 3 |
| 쿨러 | cooling | 315758 | 2 → 2 |
| 쿨러 | height | 21022 | 15 → 7 |

다나와 필터 그룹 전수: 케이스 41 / 파워 35 / 쿨러 43개. HTML을 로컬에 캐시해
두고 오프라인으로 반복 분석했다 — 파싱을 여러 번 고쳐야 하는데 그때마다
재요청하면 매너 크롤링에 어긋난다. 그룹 제목은 사이드바 검색옵션 블록의
id="SearchOption_BasicOption_{속성코드}_All" 바로 앞 btn_dic 앵커에 있음.

[✓] 케이스 max_cooler_height ↔ 쿨러 height 짝
    "이 케이스에 이 쿨러가 들어가나"를 두 화면에서 맞춰보라고 같이 넣었다.
    한쪽만 넣으면 반쪽이라 둘을 분리해서 넣지 말 것.

[✓] CASE_FORMFACTOR "ITX" = 6196-22398 은 버그가 아님 — 실측으로 확정
    다나와 라벨로는 22398=M-ITX, 22397=ITX라 틀린 것처럼 보인다. 좁혀서 대조:
      케이스크기=미니ITX 단독 40건 / +22397 → 25건(겹침 1) / +22398 → 40건(겹침 39)
    미니ITX 케이스가 실제로 지원하는 건 22398이고 사용자가 "ITX"라고 할 때
    뜻하는 것도 이쪽. 라벨만 보고 22397로 "고치면" 오히려 깨진다 —
    이 근거를 코드 주석에 그대로 박아뒀다.

[✓] 같은 속성 콤마 조인 = OR — "미지원" 미결을 데이터로 닫음(단 채택은 안 함)
    GPU_LENGTH_ATTRIBUTES 주석에 "같은 속성 구간 결합은 재실측 안 했으므로
    지금도 미지원으로 둔다"고 남아 있던 항목이다. 이번에 쟀다:
      쿨러 높이 4~6mm(20건) + ~3mm(36건), 합집합 56건
      콤마 조인 = 54건(p1 40 + p2 14, 겹침 0). B는 전부 포함, A에서 2건 누락
    같은 쿼리를 두 번 던지면 결과 집합이 완전히 동일하므로(20=20) 이 2건은
    변동이 아니라 조인이 실제로 빠뜨리는 것. 그래서 "거의 합집합이지만
    정확히는 아님"으로 결론짓고, 구간 묶기에는 쓰지 않기로 했다. 구간 필터는
    기존 GPU_LENGTH 선례대로 다나와 구간을 하나씩 노출하되 소비자 범위로
    트리밍하는 방식을 유지.

[교훈] 40건 페이지 상한 때문에 집합 비교가 무효가 되는 함정에 또 걸렸다
    조인 실측을 케이스 VGA 길이 두 구간으로 시작했는데 A·B·조인이 전부
    40건이라 아무것도 판별되지 않았다. HISTORY에 2026-08-22자로 똑같은 실패가
    이미 적혀 있는데도 같은 방식으로 출발했음. 집합을 비교하기 전에
    "상한에 걸렸나"부터 확인하고, 걸렸으면 결과가 40 미만인 좁은 값을
    고르거나 page로 넘겨서 전체를 세야 한다.

[교훈] 개발 서버가 조용히 낡은 채로 떠 있을 수 있다
    새 파라미터가 안 붙길래 코드를 의심했는데, /openapi.json의 version이
    0.17.0이었다 — uvicorn이 --reload 없이 오래 떠 있었던 것. 브랜치를
    여러 번 갈아탄 세션에서는 코드보다 서버를 먼저 의심할 것.
    확인은 curl /openapi.json | version 한 줄이면 된다.

---

## 2026-08-23 (이어서) RAM 사용 장치 필터 + 남은 백로그 종결 — backend v0.21 / frontend v0.21

남아 있던 저가치 백로그 5건을 실측했고, **1건만 채택**했다. 나머지 4건은
값어치가 없거나 애초에 존재하지 않는 필터였다 — 백로그를 실측으로 닫았다.

[✓] RAM ram_device(속성 278, 3종 전부) — RAM 필터 3 → 4
    편의가 아니라 정확성 문제다. 필터가 없을 때 DDR5 40건에
    "삼성전자 노트북 DDR5-5600"(SO-DIMM)과 "삼성전자 DDR5-5600 ECC/REG"
    (서버용)가 섞여 나온다. 그게 데스크탑 빌드에 담기면 합계·판정이 틀어진다.
    노트북용/서버용을 목록에서 빼지 않은 건 다른 카테고리의 트리밍 기준과
    다른데, 여기서는 그 값들이 "제외 대상"이 아니라 "골라서 배제하는 수단"
    이라 목록에 있어야 쓸 수 있기 때문.
    실측: 필터 없음 40건(혼입 2) → 데스크탑용 40건(혼입 0),
    노트북용 31건 전부 SO-DIMM, 두 집합 교집합 0.

[✓] 기각한 백로그 4건 — 다시 후보로 올리지 말 것
    - RAM 클럭: 다나와 RAM 검색 사이드바에 그룹 자체가 없음
    - SSD 메모리 타입(TLC/QLC): 그룹 자체가 없음. 낸드 구조는 3D/2D 2개뿐이고
      요즘 거의 전부 3D라 변별력 없음
    - CPU 코어 수: 코어 수 그룹이 없고 스레드 수(26개)만 있는데 cpu_type과 중복
    - CPU 시리즈: 코드 네임 56개(애로우레이크·그래니트 릿지…) — 정확하지만
      사용자가 모르는 이름이고 cpu_type+socket이면 세대가 사실상 정해짐
    그 외 검토 후 버린 것: CPU TDP(31개가 1W 단위로 흩어져 필터 부적합),
    기본/최대 클럭(변별력 약함), SSD 프로토콜(interface와 중복),
    SSD TBW(일반 사용자 선택 기준이 아님)

[교훈] 백로그 항목이 "존재하지 않는 것"일 수 있다
    "RAM 클럭", "SSD 메모리 타입(TLC/QLC)" 두 항목은 다나와에 필터 그룹
    자체가 없었다. 예전 세션이 다른 쇼핑몰이나 짐작으로 적어둔 것으로 보인다.
    백로그에 올릴 때 "어디서 봤는지"를 같이 적지 않으면, 다음 사람이 없는
    것을 찾느라 시간을 쓴다. 이번처럼 실측해서 "없음"을 기록해두면 그 항목은
    영구히 닫힌다.

[결과] 필터 백로그가 비었다. 8개 카테고리 커버리지:
    GPU 4 / CPU 3 / 메인보드 3 / RAM 4 / SSD 3 / 케이스 4 / 파워 4 / 쿨러 4

---

## 2026-08-24 (이어서) 인수인계 버전 표 누락분 보강 (앱 동작 변경 없음)

사용자가 "문서는 다 작업 끝났어?"라고 물어서 점검하다 발견. 각 세션 섹션
(## 오늘(2026-08-23, 이어서) ...)에는 v0.19(통계 차트)/v0.20(케이스·파워·
쿨러)/v0.21(RAM 사용 장치) 작업이 전부 기록돼 있었는데, 실가_인수인계.md의
"파일별 최신 버전" 요약 표만 backend v0.18 / frontend v0.17에서 멈춰
있었다 — PR #40~#42 세 번의 머지를 거치는 동안 그 표에만 이어 붙이질
않은 것. REFERENCE.md 산출물 파일 목록에 stats-chart-mockup.html도 빠져
있었다. 둘 다 보강(PR #43).

[교훈] "세션 섹션에 적었다"와 "요약 표에 반영했다"는 별개 작업이다
    본문 서술에는 매번 상세히 적어왔지만, 그걸 한 줄로 압축해 위쪽 요약
    표에 옮기는 건 별도 단계라 세 번 연속 빠뜨렸다. 두 번째 자리 버전을
    올릴 때는 본문 섹션 작성 + 요약 표 갱신을 한 세트로 묶어서 처리할 것 —
    본문만 쓰고 표를 다음으로 미루면 그 다음이 안 옴.
